    ...
    def get_frequecy_vector(self, sentences, filter, delimiter, dataset):
        ...
        for s in sentences:  # using delimiters to get split words
            ...
            if dataset == "Zookeeper":
                ...
                s = re.sub("=", "= ", s)
            if dataset == "Cu":
                s = re.sub(r'UE_ID', 'UE@MOD@ID', s)
                s = re.sub(r'UE_CONN', 'UE@MOD@CONN', s)
                s = re.sub(r'TRANS_ID', 'TRANS@MOD@ID', s)
                s = re.sub(r'RRC_SETUP', 'RRC@MOD@SETUP', s)
                s = re.sub(r'HANDOVER_REQUEST', 'HANDOVER@MOD@REQUEST', s)
                s = re.sub("_", " ", s)
